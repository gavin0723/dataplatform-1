...
levelHeight = function(level) {
    return 4 + Math.pow(level, 1.8) * 10;
};
...
window.drawTree = function(svgElement, conllData) {
    ...
    svg.attr('width', treeWidth + 2 * wordWidth / 3).attr('height', treeHeight + wordHeight / 2 + 20);
    words = svg.selectAll('.word').data(data).enter().append('text').text(function(d) {
        return d.word;
    }).attr('data-index', function(d) {
        if (isNaN(d.id)) return;
        return d.id;
    }).attr('class', function(d) {
        return "word cut w" + d.id + " word-" + d.tagKey;
    }).attr('x', function(d) {
        //return treeWidth - wordWidth * d.id;
        return wordWidth * d.id == 0 ? 18 : wordWidth * d.id;
    ...
    dependencies = svg.selectAll('.dependency').data(data).enter().append('text').filter(function(d) {
        ...
        return d.arrow - 7;
    }).attr('text-anchor', 'middle').attr('font-size', '55%');
